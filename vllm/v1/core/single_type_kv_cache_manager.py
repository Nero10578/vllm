...
class MambaManager(SingleTypeKVCacheManager):
    ...
    def find_longest_cache_hit(
        ...
        if drop_eagle_block and max_num_blocks > 0:
            ...
            # drops that final block as it's only partially accepted. Mamba/GDN
            # state blocks are [null, ..., state], so popping after a match
            # removes the state. Search only up to the boundary instead.
            max_num_blocks -= 1
        # Search from right to left and early stop when a match is found.
        for i in range(max_num_blocks - 1, -1, -1):
            if cached_block := block_pool.get_cached_block(
